...
HeapBlockPtr getTopHeapNode(HeapBlockPtr *actualHeap,int totalNumberofHeapNodes,CompareHeapNodes targetStrategy){
    HeapBlockPtr toRet = NULL;
    if(totalNumberofHeapNodes > 0 && actualHeap){
        HeapBlockPtr toDel = actualHeap[0];
        actualHeap[0] = actualHeap[totalNumberofHeapNodes-1];
        actualHeap[totalNumberofHeapNodes-1] = toDel;
        heapify(targetStrategy,actualHeap,totalNumberofHeapNodes-1);
        toRet=toDel;
    }
    return toRet;
}

void freeHeapNodeContents(HeapBlockPtr node){
    if (node) {
        if (node->key) {
            free(node->key);
        }
